...
integration-test: build-linux
	ginkgo -r -race -trace -cover -randomizeAllSpecs --slowSpecThreshold=60 --focus="\bINTEGRATION\b" -- --https=false
	ginkgo -r -race -trace -cover -randomizeAllSpecs --slowSpecThreshold=60 --focus="\bINTEGRATION\b" -- --https=true
	cp resources/endpoint_template.yaml $(GOOUT)/linux/endpoint.yaml

unit-test: build-linux
	ginkgo -r -race -trace -cover -randomizeAllSpecs --slowSpecThreshold=60 --focus="\bUNIT\b" -- --https=false
	ginkgo -r -race -trace -cover -randomizeAllSpecs --slowSpecThreshold=60 --focus="\bUNIT\b" -- --https=true
	cp resources/endpoint_template.yaml $(GOOUT)/linux/endpoint.yaml

test: build-linux
	ginkgo -r -race -trace -cover -randomizeAllSpecs --slowSpecThreshold=60 -- --https=false
	ginkgo -r -race -trace -cover -randomizeAllSpecs --slowSpecThreshold=60 -- --https=true
	cp resources/endpoint_template.yaml $(GOOUT)/linux/endpoint.yaml
